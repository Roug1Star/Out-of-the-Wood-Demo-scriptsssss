...
public class CameraFollow : MonoBehaviour {
    ...
    void Start()
    {
       // pivot = GameObject.Find("Player");  //找到杀手
        //pivot.transform.Find("killer");
    ...
    void LateUpdate()
    {
        if (pivot)
        {
            ...
            // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
            // apply
            Vector3 position = rotation * new Vector3(0.0f, 0.0f, -distance) + pivot.GetComponent<Transform>().position + pivotOffset;
